...
GROUPS = [ "@baogau", "@DuyetRutTeck", "@AppGameOnline9", "@ChatXocDia88", "@sanchoisangai3" ]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): user = update.effective_user user_id = str(user.id) data = load_data() if user_id not in data: data[user_id] = {"invited": [], "ref": None, "withdrawn": False, "bank": None, "name": user.full_name} save_data(data)
...
async def verify(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer() user_id = str(query.from_user.id)
...
async def handle_rut(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = str(update.effective_user.id) data = load_data() if user_id not in data or data[user_id].get("withdrawn"): await update.message.reply_text("❌ Bạn không đủ điều kiện hoặc đã rút trước đó.") return
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer() data = load_data() cb_data = query.data
...
async def extra_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer() data = load_data() user_id = str(query.from_user.id)

...
async def handle_rut(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"📥 Rút tiền từ {user_id}:
{bank_info}", reply_markup=InlineKeyboardMarkup(buttons))
        await update.message.reply_text("✅ Đã gửi yêu cầu đến Admin, vui lòng chờ!")
    except:
        await update.message.reply_text("❌ Sai cú pháp! Dùng:
